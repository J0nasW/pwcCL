...
parser = argparse.ArgumentParser(description="Papers with Code Triplet Trainer")
...
parser.add_argument(
    ...
    "--csv",
    type=str,
    help="The path to the csv file containing the papers and additional information.",
    required=True,
)
parser.add_argument(
    "-col",
...
parser.add_argument(
    ...
    "--sample_size",
    type=int,
    help="The number of papers to sample from the dataset.",
    default=None,
)
parser.add_argument(
    "-e",
...
logging.info("Welcome to the Papers with Code Triplet Trainer!")
logging.info("")
...
logging.info("Loading the papers csv file into a pandas dataframe...")
# Load the triplet csv file into a pandas dataframe
training_csv = pd.read_csv(TRAINING_CSV)

logging.info(f"Loaded {len(training_csv)} papers.")
...
logging.info(f"Got {len(training_csv)} papers with abstracts longer than 10 characters ({len(training_csv) / total_papers * 100:.2f}%).")
...
if ALLOW_DUPLICATE_CLASSES:
    ...
    logging.info(f"Exploded the dataframe. Got {len(training_csv)} papers ({len(training_csv) / total_papers * 100:.2f}%).")
else:
    ...
    logging.info(f"Kept only papers with one {COLUMN_NAME}. Got {len(training_csv)} papers ({len(training_csv) / total_papers * 100:.2f}%).")
...
# Sample the training_csv but keep the same number of papers per COLUMN_NAME so the dataset is balanced. Get a sample of SAMPLE_SIZE papers in total.
if FORCE_BALANCED_DATASET:
    ...
    logging.info(f"Got {SAMPLE_SIZE} papers after sampling with {SAMPLE_SIZE // len(areaID_set)} papers per {COLUMN_NAME} ({SAMPLE_SIZE // len(areaID_set) / len(training_csv) * 100:.2f}%).")
else:
    training_csv = training_csv.sample(SAMPLE_SIZE, random_state=42)
    logging.info(f"Got {SAMPLE_SIZE} papers after sampling randomly ({SAMPLE_SIZE / len(training_csv) * 100:.2f}%).")
